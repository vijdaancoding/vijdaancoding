...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    for index in range(len(edges)):
        ...
        print(f"[CHECK] Processing repo: {repo_name}")  # ← DEBUG output
        repo_hash, commit_count, *__ = data[index].split()

        expected_hash = hashlib.sha256(repo_name.encode("utf-8")).hexdigest()
        if repo_hash == expected_hash:
            try:
                ...
                if int(commit_count) != current_commit_count:
                    print(f"[UPDATE] Repo '{repo_name}' changed. Re-counting LOC.")  # ← UPDATE output
                    owner, repo = repo_name.split("/")
                    loc = recursive_loc(owner, repo, data, cache_comment)
                    data[index] = f"{repo_hash} {current_commit_count} {loc[2]} {loc[0]} {loc[1]}\n"
            except TypeError:
                print(f"[SKIP] Empty repo or missing data: {repo_name}")
                data[index] = f"{repo_hash} 0 0 0 0\n"
        else:
            print(f"[WARN] Hash mismatch for {repo_name}. Skipping.")
...
if __name__ == "__main__":
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2003, 8, 1))
